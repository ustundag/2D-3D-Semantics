...
if __name__ == '__main__':
    ...
    img = np.asarray(Image.open(path).resize((512,256)))/255.
    img = img[:,:,:3]
    img = img.transpose([2, 0, 1])
    img = np.expand_dims(img, 0)  # (B, C, H, W)
    print(img.shape)
    conv = SphereConv2D(3, 16, 1)
    pool = SphereMaxPool2D(1)
    out = conv(torch.from_numpy(img).float())
    x = out.detach().numpy()
    print(x.shape)
    plt.imsave('test_conv', x[0,0,:,:])
    out = pool(out)
    x = out.detach().numpy()
    print(x.shape)
    plt.imsave('test_pool', x[0,0,:,:])
    ##################### special test #############################
    """
    # test cnn
